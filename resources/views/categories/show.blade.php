        ...
        @if ($products->count())
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($products as $product)
                    <div
                        class="glass bg-white/90 shadow-xl rounded-3xl overflow-hidden flex flex-col hover:shadow-2xl transition-transform duration-200 hover:-translate-y-1">
                        ...
                        @if ($mainImage)
                            <img src="{{ asset('fyberShop/public/storage/' . $mainImage->path) }}" alt="{{ $product->name }}"
                                class="h-48 w-full object-cover">
                        @else
                            <div
                                class="h-48 w-full bg-slate-100 flex items-center justify-center text-xs font-medium text-slate-500">
