...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="mb-6 flex items-center gap-2 text-sm text-slate-600">
        ...
        <a href="{{ route('categories.show', $product->categories->first()) }}" class="hover:text-indigo-600 transition-colors">
    ...
    {{-- Main product card --}}
    <section class="relative bg-white/95 backdrop-blur-sm border-2 border-slate-200/50 shadow-2xl rounded-3xl px-6 py-8 sm:px-10 sm:py-10 overflow-hidden">
        {{-- Decorative background --}}
        <div class="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-indigo-100/30 via-purple-100/20 to-emerald-100/30 rounded-full blur-3xl -z-0"></div>
        ...
        <div class="relative flex flex-col lg:flex-row gap-10 lg:gap-12 z-10">
            ...
            <div class="lg:w-1/2 space-y-4">
                ...
                {{-- MAIN IMAGE --}}
                <div class="relative group aspect-[4/3] w-full overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 rounded-3xl shadow-xl border-2 border-slate-200/50">
            ...
            <div class="lg:w-1/2 flex flex-col justify-between gap-8">
                <div class="space-y-6">
                    {{-- Badge --}}
                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold uppercase tracking-wider">
                    ...
                    {{-- Price --}}
                    <div class="p-6 rounded-2xl bg-gradient-to-br from-indigo-50 via-purple-50 to-emerald-50 border-2 border-indigo-100">
                        <div class="flex items-baseline gap-3 mb-2">
                            <p class="text-4xl sm:text-5xl font-black bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                        ...
                        <div class="flex items-center gap-2 text-sm text-slate-600">
                            ...
                            <span>+ €{{ number_format($product->delivery_fee, 2) }} {{ __('messages.Delivery') }}</span>
                        </div>
                        @else
                        <div class="flex items-center gap-2 text-sm font-semibold text-emerald-700">
                            ...
                            <span>{{ __('messages.Free delivery') }}</span>
                        </div>
                        @endif
                    </div>
    ...
    <section class="mt-16 lg:mt-20">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 lg:gap-8">
            ...
            <div class="group product-card bg-white rounded-3xl border-2 border-slate-200/50 shadow-lg hover:shadow-2xl overflow-hidden flex flex-col backdrop-blur-sm transition-all duration-300 hover:scale-105">
                <div class="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200">
                    @if($relImage && $relImage->path)
                    <img src="{{ asset('storage/' . $relImage->path) }}"
                        alt="{{ $rel->name }}"
                    ...
                    {{-- Price badge --}}
                    <div class="absolute bottom-3 left-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-3 py-1.5 text-sm font-black shadow-xl">
                        €{{ number_format($rel->price, 2) }}
                    </div>
                </div>

                <div class="p-5 flex flex-col flex-1 bg-gradient-to-b from-white to-slate-50/50">
                    <h3 class="text-base font-bold text-slate-900 mb-3 line-clamp-2 group-hover:text-indigo-700 transition-colors">
                        {{ $rel->name }}
                    </h3>
