...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
    ...
    <div class="mb-10 text-center">
        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold uppercase tracking-wider mb-4">
    ...
    {{-- Category section --}}
    <div class="mb-10 rounded-3xl border-2 border-slate-200/50 bg-white/95 backdrop-blur-sm shadow-2xl px-6 py-8 sm:px-8 sm:py-10 overflow-hidden relative">
        {{-- Decorative background --}}
        <div class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-indigo-100/30 via-purple-100/20 to-emerald-100/30 rounded-full blur-3xl -z-0"></div>
        
        <div class="relative z-10">
            <div class="flex items-center gap-3 mb-6">
                <div class="h-10 w-10 rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-lg">
            ...
            <form action="{{ route('categories.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <input
                    type="text"
                    name="name"
                    placeholder="{{ __('messages.New category name') }}"
                    value="{{ old('name') }}"
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <form
                    ...
                    method="POST"
                    action=""
                    data-confirm-message="{{ __('messages.Delete selected category?') }}"
                    class="inline-block">
                    @csrf
                    @method('DELETE')
    ...
    {{-- Product form --}}
    <div class="relative rounded-3xl border-2 border-slate-200/50 bg-white/95 backdrop-blur-sm shadow-2xl px-6 py-8 sm:px-10 sm:py-10">
        {{-- Decorative backgrounds --}}
        <div class="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-indigo-100/30 via-purple-100/20 to-emerald-100/30 rounded-full blur-3xl -z-0 overflow-hidden"></div>
        <div class="absolute bottom-0 left-0 w-64 h-64 bg-gradient-to-tr from-emerald-100/20 via-indigo-100/20 to-purple-100/20 rounded-full blur-3xl -z-0 overflow-hidden"></div>
        ...
        <div class="relative z-10">
            ...
            @if (session('success'))
            <div class="mb-6 rounded-2xl border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 to-green-50 px-6 py-4 text-sm font-semibold text-emerald-800 text-center shadow-lg animate-fadeInUp flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 relative" id="product-form" style="overflow: visible;">
            @csrf
            ...
            <div>
                ...
                <textarea
                    ...
                    name="description"
                    rows="5"
                    placeholder="{{ __('messages.Describe the product') }}"
    ...
    "
                ...
                : chooseText"
                    ...
                    <svg class="h-5 w-5 text-slate-500 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div
                    ...
                    @foreach($categories as $cat)
                    <label class="flex items-center gap-3 cursor-pointer p-2 rounded-lg hover:bg-indigo-50 transition-colors">
                        <input
                            type="checkbox"
                            name="categories[]"
                            value="{{ $cat->id }}"
