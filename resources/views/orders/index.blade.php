                ...
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-slate-100 mb-4">
                    <svg class="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 3h18v4H3V3zm0 6h18v12H3V9zm6 3h6"></path>
                    </svg>
                </div>
        ...
        @else
            ...
            <div class="space-y-4">
                @foreach ($orders as $order)
                    <div
                        class="group rounded-3xl border-2 border-slate-200/50 bg-white/90 backdrop-blur-sm p-6 shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.01]">
                ...
                @class([
                    ...
                    'bg-slate-100 text-slate-700' => $order->payment_status === 'unpaid',
                                ...
                                {{-- Retry payment --}}
                                @if (in_array($order->payment_status, ['pending', 'failed']) && $order->payment_method !== 'cod')
