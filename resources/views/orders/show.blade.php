        ...
        <div class="grid sm:grid-cols-2 gap-6">
            <div class="space-y-4">
                <div class="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                    <p class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">{{ __('messages.Payment method') }}</p>
                    <p class="text-base font-bold text-slate-900">{{ strtoupper($order->payment_method) }}</p>
                </div>

                <div class="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                    <p class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">{{ __('messages.Order Status') }}</p>
                    <p class="text-base font-bold text-slate-900">{{ ucfirst($order->order_status) }}</p>
                </div>
            </div>

            <div class="space-y-4">
                <div class="rounded-2xl border-2 border-indigo-100 bg-gradient-to-br from-indigo-50 via-purple-50 to-emerald-50 p-4">
                    <p class="text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">{{ __('messages.Total') }}</p>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-4">
                    <p class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">{{ __('messages.Shipping address') }}</p>
                    @if($order->address)
